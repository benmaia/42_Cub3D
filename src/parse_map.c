...
/*   Updated: 2023/03/15 20:52:15 by dmarceli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_valid(t_mlx *g)
{
	...
	while (g->m->cub[++i])
	{
		...
		&& ft_strncmp(g->m->cub[i], "SO ", 3)
		&& ft_strncmp(g->m->cub[i], "WE ", 3)
		&& ft_strncmp(g->m->cub[i], "EA ", 3)
		&& ft_strncmp(g->m->cub[i], "F ", 2)
		&& ft_strncmp(g->m->cub[i], "C ", 2)
		&& ft_strcmp(g->m->cub[i], "\n"))
		if (g->m->cub[i][0] != '1'
		&& g->m->cub[i][0] != ' ')
			wrong_map(g);
